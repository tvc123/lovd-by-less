class SessionsController < ApplicationController
            ...
            if result.successful?
                ...
                if @user.new_record?
                    if !registration['nickname'].blank? && !registration['email'].blank?
                        ...
                        flash[:error] = "Your account must include at a minimum a nickname and valid email address to use OpenID on this site."
                        render :action => 'new'
                    end
                else
    ...
    def create_open_id_user(user)
        ...
        flash[:error] = "Someone has signed up with that nickname or email address. Please select a diferent username and/or use a different email address."
        render :action => 'new'
    end    
    ...
    def password_authentication(login, password)
        ...
        if user == nil
            failed_login("We're sorry, but we couldn't recognize your login information.  Please try again.")
