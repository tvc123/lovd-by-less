class CustomFormBuilder < ActionView::Helpers::FormBuilder
    ...
    def render_field_template(name, field, options)
        ...
        if options[:hide_required] 
            required = false
            label_text = options[:label]
        else
            required = required_field?(field)
            label_text = options[:label] + required_mark(field)
        end
    ...
    # TODO maybe move this into a different builder?
    ...
    def required_mark(field)
        required_field?(field) ? " <em>(required)</em>" : ''
    end
